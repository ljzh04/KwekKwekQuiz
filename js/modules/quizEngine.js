 ...
 * @type {Array}
 * @private
 * @description Stores the original quiz content for re-shuffling on restart
 */
let originalQuizContent = [];
...
export function startQuiz(quizContent) {
    ...
    // Store original quiz content for re-shuffling on restart
    originalQuizContent = JSON.parse(JSON.stringify(quizContent)); // Deep copy to avoid mutations
...
export function restartCurrentQuiz() {
    // Reapply randomization based on settings using the original quiz content
    if (originalQuizContent.length > 0) {
        let randomizedQuizContent = [...originalQuizContent]; // Create a copy from original
